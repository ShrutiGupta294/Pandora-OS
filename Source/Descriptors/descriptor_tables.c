...
void init_gdt()  // For BSP
 ...
 uintptr_t pmode_GDT_init(uint32_t APIC_id)
 {
   ...
   pmode_GDT_lgdt(APIC_id, gdt_new);
   return gdt_new;
 }

 uintptr_t pmode_IDT_init(uint32_t APIC_id)   // Create IDT and save it
 {
   ...
   pmode_IDT_lidt(APIC_id, idt_new);
   return idt_new;
 }

 uintptr_t pmode_IDT_initP(uint32_t APIC_id)    // Create Only IDT; DOnt save it
 {
   uintptr_t idt_new = (uintptr_t)desc_malloc(960);//(vector_addr + AP_startup_Code_sz + pmode_code_size + 16);
   AP_idt_Setup((uint32_t*)(idt_new + 8), (uint32_t*)idt_new);//(vector_addr + AP_startup_Code_sz + pmode_code_size + 8 + 16), gdt_new);
   return idt_new;
 ...
 void pmode_GDT_lgdt(uint32_t APIC_id, uintptr_t gdt_new) // Save the gdt to processor
 {
   if(APIC_id == 0)
   {
     lgdt((void *)gdt_new);
   ...
   else
   {
     ...
     // Send Command to AP to Load new gdt by replacing place holder number with gdt's address
   }
 }

 void pmode_IDT_lidt(uint32_t APIC_id, uintptr_t idt_new)   // Save the IDT to processor
 {
   if(APIC_id == 0)
   {
      lidt((void *)idt_new);
   ...
   else
   {
     ...
     ByteSequence_Replace(0x32409799, 4, (uint32_t)idt_new, 4, (uint32_t*)pmode_code_addr, (uint32_t*)(pmode_code_addr + pmode_code_size));
      // Send Command to AP to Load new idt by replacing place holder number with idt's address
    }
 }
